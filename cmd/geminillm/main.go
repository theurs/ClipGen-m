...
func init() {
	flag.Var(&flagFiles, "f", "Файлы")
	flag.StringVar(&flagSystem, "s", "", "Системный промпт")
	flag.BoolVar(&flagJson, "j", false, "JSON режим")
	flag.StringVar(&flagMode, "m", "auto", "Режим")
	flag.Float64Var(&flagTemp, "t", -1.0, "Температура")
	flag.BoolVar(&flagVerbose, "v", false, "Verbose")
	flag.StringVar(&flagSaveKey, "save-key", "", "Сохранить ключ")
	flag.StringVar(&flagChatID, "chat", "", "ID чата")
}
...
func main() {
	...
	filesData, hasImages, _, hasPdf := processFiles(flagFiles)
	if userPrompt == "" && len(filesData) == 0 {
		fatal("Отсутствуют входные данные (stdin или файлы).")
	}

	mode := determineMode(flagMode, userPrompt, hasImages, hasPdf)
	...
	for _, apiKey := range shuffledKeys {
		...
		for _, modelName := range modelsList {
			...
			if flagChatID != "" {
				chatHistory, _ = loadChatHistory(flagChatID)
			}

			result, errReq := requestGemini(apiKey, cfg.BaseURL, modelName, finalSystem, userPrompt, filesData, finalTemp, flagJson, chatHistory)

			if errReq == nil {
				// Успех
				if flagChatID != "" && chatHistory != nil {
					saveHistory(flagChatID, chatHistory, userPrompt, result, cfg)
				}
