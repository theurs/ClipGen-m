...
1. Скачайте исполняемый файл `pollinationsllm.exe` (иногда сокращенно `plnllm`)
...
echo "Привет! Как дела?" | pollinationsllm

# Запрос с файлом
pollinationsllm -f файл.txt

# Запрос с изображением
pollinationsllm -f фото.jpg

# Запрос с несколькими файлами
pollinationsllm -f файл1.txt -f изображение.jpg
```
...
echo "Объясни смысл жизни" | pollinationsllm

# Режим кода
echo "Объясни этот код" | pollinationsllm -m code

# Режим зрения (для изображений)
pollinationsllm -f image.jpg -m vision

# Режим аудио (для аудио файлов)
pollinationsllm -f audio.mp3 -m audio

# Режим OCR (для PDF/изображений с текстом)
pollinationsllm -f document.pdf -m ocr
```
...
echo "Чему равно 25 * 36 + 17 * 42?" | pollinationsllm

# Калькулятор: Алгоритмы Lua
echo "Вычисли факториал 15" | pollinationsllm

# Калькулятор: Программы на Lua
echo "Напиши Lua-скрипт, который сортирует массив [5, 2, 8, 1, 9]" | pollinationsllm

# Поиск: Веб-поиск через Tavily
echo "Какие последние новости о языке программирования Go?" | pollinationsllm

# Поиск: Рыночные данные
echo "Какова цена акций Apple сегодня?" | pollinationsllm

# Поиск: Через модель gemini-search на Pollinations
echo "Найди актуальную информацию о состоянии API Pollinations" | pollinationsllm

# Смешанные запросы
echo "Найди курс доллара и вычисли, сколько будет 100 долларов в рублях" | pollinationsllm
```
...
```bash
# Начать новый чат
echo "Привет, как дела?" | pollinationsllm -chat мой_чат

# Продолжить чат (история сохраняется)
echo "А что ты умеешь?" | pollinationsllm -chat мой_чат

# Задать вопрос с контекстом из истории
echo "Напомни, о чем мы говорили?" | pollinationsllm -chat мой_чат

# Очистить историю чата
pollinationsllm -clear-chat мой_чат

# Очистить чат командой в сообщении
echo "/clear" | pollinationsllm -chat мой_чат
```
...
echo "Создай JSON с информацией о пользователе" | pollinationsllm -j
```
...
pollinationsllm -save-key ваш_pollinations_api_ключ

# Добавить Tavily API ключ (для инструмента поиска)
pollinationsllm -add-tavily-key tvly-abcdefghijklmnopqrstuvwxyz1234567890
```
...
2. **"Нет API ключей"**: Добавьте хотя бы один ключ с помощью `pollinationsllm -add-tavily-key ВАШ_КЛЮЧ` (Pollinations может работать без ключа)
...
```bash
go build -o pollinationsllm.exe main.go
```
