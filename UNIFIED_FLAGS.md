...
- `-f` / `--f` / `--file` - указание файлов для обработки (можно использовать несколько раз). Поддерживаемые типы файлов зависят от конкретной утилиты: изображения (все утилиты), текстовые файлы (все утилиты), аудио (ограниченно поддерживается в geminillm, полноценно в mistral, ghllm и groqllm), PDF (через OCR в mistral)
